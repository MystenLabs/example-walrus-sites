...
export async function getFlatlander(
  objectId: string
): Promise<Flatlander | null> {
  console.log("getting the flatlander");
  ...
  if (obj.data && obj.data.bcs && obj.data.bcs.dataType === "moveObject" && obj.data.type === flatlanderType()) {
...
export async function drawFlatlander(flatlander: Flatlander) {
  console.log("drawing the flatlander");
  // Draw the flatlander
  const radius = 50;
  const centerX = 100;
  const centerY = 100;
...
function main() {
  ...
  getFlatlander(objectId).then((flatlander) => {
    ...
    line1.innerHTML = "the color and the number of sides is taken from the NFT;</br>this URL is unique to this Flatlander, and always will be.";
